    ...
    public class InternetRoutingProxy
    {
        ...
        public void StartProxy(IPEndPoint localEndPoint, IPEndPoint remoteEndPoint)
        {
            ...
            while (true)
            {
                ...
                source.BeginReceive(forwardingInformation.Buffer, 0, forwardingInformation.Buffer.Length, 0,
                    OnDataReceive, forwardingInformation);
            }
        }

        public void StartProxy(IPEndPoint localEndPoint, Socket existingRemoteConnection, bool throwOnException = true)
        {
            _baseSocket.Bind(localEndPoint);
            _baseSocket.Listen(10);
            ...
            while (true)
            {
                ...
                var forwardingInformation = new ForwardingInfo(source, destination._baseSocket) { ThrowOnError = throwOnException };
        ...
        public void StartProxy(Socket existingLocalConnection, IPEndPoint remoteEndPoint)
        {
            ...
            var forwardingInformation = new ForwardingInfo(source, destination._baseSocket);
            destination.Connect(remoteEndPoint, source);
            source.BeginReceive(forwardingInformation.Buffer, 0, forwardingInformation.Buffer.Length, 0, OnDataReceive,
        ...
        private void Connect(EndPoint remoteEndpoint, Socket destination)
        {
            ...
            _baseSocket.BeginReceive(forwardingInformation.Buffer, 0, forwardingInformation.Buffer.Length,
                SocketFlags.None, OnDataReceive, forwardingInformation);
        }

        private void ConnectWithExisting(Socket destination, bool throwOnError = true)
        {
            var forwardingInformation = new ForwardingInfo(_baseSocket, destination) { ThrowOnError = throwOnError};
            _baseSocket.BeginReceive(forwardingInformation.Buffer, 0, forwardingInformation.Buffer.Length,
                SocketFlags.None, OnDataReceive, forwardingInformation);
        }
        ...
        private static void OnDataReceive(IAsyncResult result)
        {
            ...
            try
            {
                ...
                if (bytesRead > 0)
                {
                    forwardingInformation.DestinationSocket.Send(forwardingInformation.Buffer, bytesRead,
                        SocketFlags.None);
                    forwardingInformation.SourceSocket.BeginReceive(forwardingInformation.Buffer, 0,
                        forwardingInformation.Buffer.Length, 0, OnDataReceive, forwardingInformation);
                }
            }
            catch (Exception)
            {
                ...
                if (forwardingInformation.ThrowOnError)
                    throw;
            }
        }
    }
